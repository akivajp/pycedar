...
トライはキーを配列に符号化して保持するため、キーごとの文字列オブジェクトを
別途持つ必要がありません。またハッシュマップにはできない接頭辞検索に答えられます。

4〜16 文字のランダムな小文字キー 200000 件を整数に対応付けた場合の実測値
（`benchmarks/bench.py`、CPython 3.13、Linux x86_64）:
...
| tail 配列 | — | 2.06 MB |
| **合計** | **23.16 MB** | **4.40 MB** |
| 1キーあたり | 121 バイト | 23 バイト |

一方で引き換えになるものも明確です。**単一キーの検索は `dict` のおよそ 2 倍の
時間がかかります**（同じデータで 30 ns 対 16 ns。数値は環境により変動します）。キー数が多くメモリが問題に
なる場合や、接頭辞検索が必要な場合に pycedar を選び、単一キーの検索しか
必要ないなら `dict` を選ぶのが妥当です。

手元のデータで再現するには `python benchmarks/bench.py --help` を参照してください。
...
結果は保証されません。遅延の効果はどこで打ち切るか次第です。上の表と同じ
20000 キーで ``benchmarks/bench.py`` により計測したところ、2 文字の predict で
先頭 1 件のみの取得は全件 list 化の約 1/10 (プレフィックスあたり 332 ns に対し
2892 ns) でした。一方、ジェネレータを最後まで消費すると list 版より約 1 割
遅くなります。search の結果はキー 1 バイトあたり最大 1 件に限られるため、
どちらの形でも安価です。
...
### 並行アクセスが安全なのは別々のトライの間のみ

モジュールはフリースレッド互換（PEP 703）を宣言しており、フリースレッド版の
CPython 3.14 で動作します。これは *別々の* トライオブジェクトを複数スレッド
から同時に使うことをカバーしており、テストスイートでも検証しています。

一方で、個々のトライがスレッドセーフになったわけではありません。*同一の*
トライへの並行操作は GIL 時代と同様に競合し、破壊され得ます。共有のトライを
扱う場合は、他の可変オブジェクトと同様にロックで保護してください。
...
cedar はキーの短い接尾辞を NUL 終端の配列に保持するため、キー自体が NUL を
含むとその不変条件が壊れます。そのようなキーは拒否されます。
...
0.4.0 より前は受理されていました。しかも読み出しが狂うだけでは済まず、
このようなキーを入れた後に接頭辞を共有するキーを入れると、メモリが破壊され
インタプリタごと落ちていました。
